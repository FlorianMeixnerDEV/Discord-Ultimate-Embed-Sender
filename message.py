...
def load_config():
    default_config = {"webhooks": {}, "last_selected": ""}
            ...
            with open(CONFIG_FILE, "r") as f:
                ...
                if "webhooks" not in data:
                    if "webhook_url" in data and data["webhook_url"]:
                        return {"webhooks": {"#standard": data["webhook_url"]}, "last_selected": "#standard"}
                    return default_config
                return data
        except Exception as e:
...
def update_dropdown():
    ...
    config = load_config()
    channels = list(config["webhooks"].keys())
...
def open_markdown_help():
    ...
    tk.Label(help_win, text=get_txt("help_note"), font=("Arial", 8, "bold", "italic"), fg="#faa61a", bg="#2f3136").pack(pady=10)

def open_settings():
    settings_win = tk.Toplevel(root)
    settings_win.title(get_txt("settings_title"))
    settings_win.geometry("480x360")
...
def send_webhook():
    ...
    webhook_url = config["webhooks"].get(selected_channel)
    
    if not webhook_url or selected_channel == get_txt("no_channels"):
